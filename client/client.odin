...
main :: proc() {
	...
	my_turn := init.state.my_turn
	ready := init.state.ready
	state := init.state.state
	log.infof("Connected with %v.", player_id)
	...
	if s, playing := state.(common.PlayState); playing {
		for i in 0 ..= 2 {
			for j in 0 ..= 2 {
				if s.board[i][j] != .None {
	...
	for !rl.WindowShouldClose() {
		...
		for p in payloads {
			...
			gstate := spl.(common.GameState) or_continue

			my_turn = gstate.my_turn
			ready = gstate.ready
			state = gstate.state
		...
		case common.WinState:
			if (mouse_pos.y < HEADER_HEIGHT && rl.IsMouseButtonPressed(rl.MouseButton.LEFT)) ||
			   rl.IsKeyPressed(rl.KeyboardKey.ENTER) {
				...
				send_err := send(
					socket,
					common.ClientPayload{player_id = player_id, type = common.Restart{}},
				)
				if send_err != nil {
					log.errorf("Failed to send data to server: %v.", send_err)
				}
		...
		render_header(&state, bool(my_turn), bool(ready))
		render_grid()
	...
	send(socket, common.ClientPayload{player_id = player_id, type = common.Disconnect{}})
